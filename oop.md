...
Выделяют три вида полиморфных функций:

+ _классический_, если разные классы имплементируют одинаковый интерфейс или наследуются от одного класса, их объекты будут вести себя одинаково. 
+ _динамический ad hoc_, тесно связан с наследованием и заключается в переопределении (англ. overriding) методов — он позволяет им демонстрировать различное поведение при вызове для разных типов;
+ _статический ad-hoc_, это перегрузка (англ. overloading) метода. В классе может быть несколько методов с одинаковым названием, но с разными типами параметров. В этом случае компилятор сам выберет нужную реализацию — в зависимости от типа переданного аргумента, который становится известен на этапе компиляции;
+ _параметрический_, вид полиморфизма, позволяющий реализовать поведение класса или метода с указанием параметров и возвращаемых значений общего вида вместо конкретных.
